	...
	if ($request_ === 'GET')
	{
		...
		switch ( $action2perform_ ) {
			case 1 : {
				...
				$id_ = @intval($_GET['id_']);
				if (!is_numeric($id_) || $id_ < 1000) {
					http_response_code(403);
					header(STANDARD_HEADER);
					die( json_encode([ 'error' => 'parameters error on ID requested [ERROR]' ]));
				}
				$rs = (new ClassModel($connection2db_))->getContactData($id_, $page_, $numberOf_);
				$HTTP_STATUS = 200;
				break;
			}
			case 2 : { 
				...
				$rs = (new ClassModel($connection2db_))->getContactDataById($id_);
				$HTTP_STATUS = 200;
				break;
			}
			case 3 : { 
				...
				$rs = (new ClassModel($connection2db_))->getContactDataByValue($querystring2search_);
				$HTTP_STATUS = 200;
				break;
			}
			default : {
